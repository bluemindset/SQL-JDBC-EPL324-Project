...
INSERT INTO @eighthorse 
 ...
 SET @HorseCursor = CURSOR FOR
	...
	Insert into @horset(meeting_date,horse_id,race_time,end_pos,distance,pos1,pos2,pos3)
		SELECT   [PARTICIPATION].meeting_date ,[PARTICIPATION].horse_id,[Race].Race_time, [PARTICIPATION].end_pos, RACE.distance,	
		...
		GROUP BY [PARTICIPATION].meeting_date , [PARTICIPATION].horse_id, [PARTICIPATION].end_pos,RACE.distance,[Race].race_time 
    ...
    INTO @horse_idt,@meeting_datet,@race_timet,@distancet,@end_post,@pos1t,@pos2t,@pos3t
   ...
   SET @same_horse = @horse_idt;
    ...
    BEGIN  
		...
		ELSE
			...
			Insert INTO @horset2
			SELECT @horse_idt , @meeting_datet ,@race_timet,@distancet ,@end_post, @pos1tf ,@pos2tf,@pos3tf
			,@c ,@perc
